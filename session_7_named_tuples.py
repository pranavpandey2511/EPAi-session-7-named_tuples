...
def timer(n_iter: int) -> 'Decorator':
    ...
    def timed_inner(fn: callable) -> 'closure':
        ...
        def inner(*args: 'arguments', **kwargs: 'keyword arguments'):
            ...
            return result, f'Average Time to Run the Function is: {(end-start)/n_iter :.3f}'
...
def nt_approach(num_profiles: int):
    ...
    result = Stat(max_age, highest_blood, mean_location, average_age)
...
def dict_approach(num_profiles: int):
    ...
    return {'oldest_person': max_age, 'highest_blood': highest_blood, 'mean_location': mean_location, 'average_age': average_age}
